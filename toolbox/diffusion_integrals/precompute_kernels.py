...
therm_path = os.path.join(out_dir, "therm_dim.dat")
...
def thermal_kernel(r_vals, z_vals, R: float, kappa: float, P: float,
                   L: float) -> np.ndarray:
    """Epithermal/thermal kernel: Dirichlet axial factor (Eqs. 26-27)."""
    r_vals = np.asarray(r_vals, dtype=float)
    z_vals = np.asarray(z_vals, dtype=float)

    alpha = np.hypot(k, kappa)
    A_ax = axial_factor_thermal_bc(z_vals, P=P, L=L, alpha=alpha)
...
def main():
    ...
    therm_dim = np.memmap(
        therm_path, dtype="float32", mode="w+", shape=(NP, NK, NZ, NR)
    )
    ...
    for iP, P_hat in enumerate(P_HAT_GRID):
        ...
        for ik, kappa_hat in enumerate(KAPPA_HAT_GRID):
            kappa_phys = kappa_hat / R

            print(
                f"P_hat={P_hat:5.2f} (P={P_phys:6.2f} cm), "
                f"kappa_hat={kappa_hat:5.2f} (kappa={kappa_phys:5.2f} 1/cm)"
            )

            I_ev = evap_kernel(
                r_phys, z_phys,
                R=R, kappa=kappa_phys, P=P_phys,
            )

            I_th = thermal_kernel(
                r_phys, z_phys,
                R=R, kappa=kappa_phys, P=P_phys, L=L,
            )

            # storage order: (P_hat, kappa_hat, xi, rho)
            evap_dim[iP, ik, :, :] = I_ev.T.astype("float32")
            therm_dim[iP, ik, :, :] = I_th.T.astype("float32")
