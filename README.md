...
Manuscript figures are generated by scripts under `toolbox/figures/`, which
compute model values from `fitting_params/`. The equivalent cells in
`plots.ipynb` read `{species}_model.npy`; those arrays were written by Option 1
and are **stale**, so prefer the scripts:

```
python toolbox/figures/spectral_fluence_comp.py     # writes to FIGURES_DIR or ./figures
```
